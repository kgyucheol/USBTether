...
방화벽은 "이 cgroup에서 나온 트래픽만" 폰으로 보낸다.
  ...
  user.slice/user-<uid>.slice/user@<uid>.service/phonesocks.slice
      ├── phonesocks-holder.service     slice 를 살아있게 유지 (nft 규칙이 경로를 요구)
...
SLICE = "phonesocks.slice"
HOLDER_UNIT = "phonesocks-holder.service"
